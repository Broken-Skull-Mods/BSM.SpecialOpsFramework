/*
	Author: Jiri Wainar & BloodyMystik
	...
	* Unit automatically leaves the animation loop if there is an enemy in 300m he knows about.
	...
	Example:
	[this,"STAND","FULL",{(player distance _this) < 5}] call SpecOps_fnc_a3AmbientAnimCombat;
*/
...
private _savedAIState = createHashMap;
private _allAIFeatures = [
    "ANIM","AUTOTARGET","FSM","MOVE","TARGET",
    "AIMINGERROR","AUTOCOMBAT","CHECKVISIBLE","COVER","LIGHTS",
    "MINEDETECTION","NVG","RADIOPROTOCOL","PATH","RADIOPROTOCOL",
    "SUPPRESSION","TARGET","TEAMSWITCH","WEAPONAIM"
];
{_savedAIState set [_x, _unit checkAIFeature _x];} forEach (_allAIFeatures);
...
[_unit,_cond,_transAnim,_behaviour, _savedAIState, _allAIFeatures] spawn
{
	private["_unit","_cond","_transAnim","_animHandle","_behaviour","_unitPos","_savedAIState" ,"_allAIFeatures"];

	_unit 		= _this select 0;
	_cond 		= _this select 1;
	_transAnim 	= _this select 2;
	_behaviour 	= _this select 3;
	_savedAIState 	= _this select 4;
	_allAIFeatures 	= _this select 5;
    ...
    {
        private _isEnable = _savedAIState get _x;
        if (_isEnable) then {
            _unit enableAI _x;
        }else {
            _unit disableAI _x;
        };
    } forEach (_allAIFeatures);
