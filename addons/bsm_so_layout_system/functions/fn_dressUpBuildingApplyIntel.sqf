params ["_building", "_buildingInfo", ["_directInfo", nil]];
...
private _selectedIntelSpot = nil;
if (isNil "_directInfo") then {
	 private _loadedFurniture = [_buildingInfo, "intelFurnitures"] call SpecOps_fnc_dressUpBuildingGetBuildingSection;
	 _selectedIntelSpot = selectRandom _loadedFurniture;
} else {
	_selectedIntelSpot = _directInfo;
};
