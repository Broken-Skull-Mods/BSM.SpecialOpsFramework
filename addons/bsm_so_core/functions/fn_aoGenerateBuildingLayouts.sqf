    ...
    */
	diag_log format ["SPEC-OPS (AO): _area = %1 | _areaHalfSize = %2 | _totalEnemies = %3 | _leaderNb = %4 | _intelMax = %5 | _staticGunnerCount = %6 | _hostagesNb = %7", 
	...
	*/
			...
			if(_index >= 0) then  {
				...
				if(_index >= 0) then  {
					...
					for "_i" from 1 to _intelGuardMax do {
						...
						private _buildingUnitSpawned = [_randomBuildingObject, _randomBuildingUnitPosSet, _prioritizeLeaders, (createGroup east), _faction, _factionCamo] call SpecOps_fnc_dressUpBuildingSpawnEnemy;
		...
		for "_i" from 1 to _hostagesNb do {
			...
			if(_index >= 0) then  {
				...
				for "_i" from 1 to _randomGuards do {
					...
					diag_log format ["SPEC-OPS (AO):  %1 Guards for the Hostage.", _randomGuards];
					_randomBuildingUnitPositions = _randomBuildingUnitPositions - [_randomBuildingUnitPosSet];
					private _buildingUnitSpawned = [_randomBuildingObject, _randomBuildingUnitPosSet, false, (createGroup east), _faction, _factionCamo] call SpecOps_fnc_dressUpBuildingSpawnEnemy;
	...
	if (_garrisonEnemies > 0 && count _buildingsAndUnitPositions > 0) then {
		for "_i" from 1 to _garrisonEnemies do {
			...
			private _buildingUnitSpawned = [_randomBuildingObject, _randomInfoSet, false, (createGroup east), _faction, _factionCamo] call SpecOps_fnc_dressUpBuildingSpawnEnemy;
			private _groupExistIndex = _garrisonGroups findif { _x select 0 == _randomBuildingObject; };
			if (_groupExistIndex == -1) then { 
				_garrisonGroups pushback [(_buildingUnitSpawned select 0), (_buildingUnitSpawned select 2)]; 
			} else {
			...
			diag_log format ["SPEC-OPS (AO): Spreading Garrison."];
			missionNamespace setVariable ["tft_global_loading_status", format['Spawning Enemies (%1)', ([1000, 9999] call BIS_fnc_randomInt)], true];
    		call SpecOps_fnc_commonSleepFPSBased;
		};
		call SpecOps_fnc_commonSleepFPSBased;
	...
	if (_leaderNb > 0 && count _buildingsAndUnitPositions > 0) then {
		...
		for "_i" from 1 to _leaderNb do {
			...
			private _randomOptions = _randomInfoSet select 2;
			private _group = objNull;
			private _buildingUnitSpawned = [_randomBuildingObject, _randomInfoSet, true, (createGroup east), _faction, _factionCamo] call SpecOps_fnc_dressUpBuildingSpawnEnemy;
			private _groupExistIndex = _garrisonGroups findif { _x select 0 == _randomBuildingObject; };
			if (_groupExistIndex == -1) then { 
				_garrisonGroups pushback [(_buildingUnitSpawned select 0), (_buildingUnitSpawned select 2)]; 
			} else {
...

	{
		...
		missionNamespace setVariable ["tft_global_loading_status", format['Spawning Garrison Layouts (%1)', ([1000, 9999] call BIS_fnc_randomInt)], true];
		call SpecOps_fnc_commonSleepFPSBased;
	} forEach (_nearbyBuildings);
